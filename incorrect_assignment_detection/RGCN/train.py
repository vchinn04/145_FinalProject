    ...
    for epoch_num in range(args.epochs):
        ...
        for tmp_train in tqdm(train_data):
            ...
            if (coorg_idx.shape[0] <= 0):
                coorg_et = torch.tensor([],  dtype=torch.int64)
            else:
                coorg_et = torch.zeros(coorg_idx.shape[1],  dtype=torch.int64)

            if (coauth_idx.shape[0] <= 0):
                coauth_et = torch.tensor([],  dtype=torch.int64)
            else:  
                coauth_et = torch.full_like(torch.zeros(coauth_idx.shape[1], dtype=torch.int64), 1)

            if (covenue_idx.shape[0] <= 0):
                covenue_et = torch.tensor([],  dtype=torch.int64)
            else:
                covenue_et = torch.full_like(torch.zeros(covenue_idx.shape[1],  dtype=torch.int64), 2)
