...
def getdata(orcid):
    ...
    for ii in range(len(all_pappers_id)):
        ...
        for jj in range(len(all_pappers_id)):
            ...
            if w_coauthor + w_coorg + w_covenue == 0:
                ...
                if (w_coorg > 0):
                    coorg_matrix.append([paper1_id, paper2_id])

                if (w_covenue > 0):
                    covenue_matrix.append([paper1_id, paper2_id])
    ...
    coauthor_matrix = [[re_num[i],re_num[j]] for i,j in coauthor_matrix]
    coauthor_index = np.array(coauthor_matrix, dtype=np.int64).T

    coorg_matrix = [[re_num[i],re_num[j]] for i,j in coorg_matrix]
    coorg_index = np.array(coorg_matrix, dtype=np.int64).T

    covenue_matrix = [[re_num[i],re_num[j]] for i,j in covenue_matrix]
    covenue_index = np.array(covenue_matrix, dtype=np.int64).T
